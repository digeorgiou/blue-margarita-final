...
public class ProcedureRestController {
    ...
    @Operation(
            summary = "Get all active procedures",
            description = "Retrieves all active procedures without pagination. Used for simple listings and exports.",
            responses = {
                    @ApiResponse(
                            responseCode = "200",
                            description = "List of all active procedures",
                            content = @Content(
                                    mediaType = "application/json",
                                    schema = @Schema(implementation = ProcedureReadOnlyDTO.class)
                            )
    ...
    @GetMapping("/active")
    @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
    public ResponseEntity<List<ProcedureReadOnlyDTO>> getAllActiveProcedures() {
        List<ProcedureReadOnlyDTO> procedures = procedureService.getAllActiveProcedures();
        return new ResponseEntity<>(procedures, HttpStatus.OK);
    }

    @Operation(
            summary = "Get filtered procedures without pagination",
            description = "Retrieves procedures based on filter criteria without pagination. Used for exports and reports.",
            responses = {
                    @ApiResponse(
                            responseCode = "200",
                            description = "List of procedures matching filter criteria",
                            content = @Content(
                                    mediaType = "application/json",
                                    schema = @Schema(implementation = ProcedureReadOnlyDTO.class)
                            )
    ...
    @GetMapping("/filtered")
    @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
    public ResponseEntity<List<ProcedureReadOnlyDTO>> getFilteredProcedures(
            @Parameter(description = "Procedure name filter") @RequestParam(required = false) String name,
            @Parameter(description = "Active status filter") @RequestParam(required = false) Boolean isActive) {
        ...
        List<ProcedureReadOnlyDTO> procedures = procedureService.getFilteredProcedures(filters);
        return new ResponseEntity<>(procedures, HttpStatus.OK);
    }
}