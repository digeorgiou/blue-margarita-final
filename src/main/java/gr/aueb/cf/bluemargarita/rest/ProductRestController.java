...
public class ProductRestController {
    ...
    @Operation(
            summary = "Get products for stock management",
            description = "Retrieves products optimized for stock management operations with lightweight data. Used by dedicated stock management page.",
            responses = {
                    @ApiResponse(
                            responseCode = "200",
                            description = "Paginated list of products with stock information",
                            content = @Content(
                                    mediaType = "application/json",
                                    schema = @Schema(implementation = Paginated.class)
                            )
    ...
    public ResponseEntity<Paginated<StockManagementDTO>> getProductsForStockManagement(
        ...
        ProductFilters filters = ProductFilters.builder()
                .nameOrCode(nameOrCode)
                .isActive(true) // Only active products for stock management
                .build();
        ...
        Paginated<StockManagementDTO> products = productService.getProductsForStockManagement(filters);
        return new ResponseEntity<>(products, HttpStatus.OK);
    }

    @Operation(
            summary = "Update product stock",
            description = "Updates the stock for a single product with comprehensive audit logging. Supports ADD, REMOVE, and SET operations.",
            responses = {
                    @ApiResponse(
                            responseCode = "200",
    ...
    public ResponseEntity<StockUpdateResultDTO> updateProductStock(
        ...
        StockUpdateResultDTO result = productService.updateProductStock(stockUpdateDTO);
        return new ResponseEntity<>(result, HttpStatus.OK);
    }

    @Operation(
            summary = "Update multiple products stock",
            description = "Updates stock for multiple products in a single atomic transaction. Used for bulk stock operations.",
            responses = {
                    @ApiResponse(
                            responseCode = "200",
    ...
    public ResponseEntity<List<StockUpdateResultDTO>> updateMultipleProductsStock(
            @Valid @RequestBody BulkStockUpdateDTO bulkStockUpdateDTO,
        ...
        List<StockUpdateResultDTO> results = productService.updateMultipleProductsStock(bulkStockUpdateDTO);
    ...
    @Operation(
            ...
            description = "Retrieves products with stock below their low stock alert threshold. Used for dashboard widgets.",
            responses = {
                    @ApiResponse(
                            responseCode = "200",
    ...
    @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
    public ResponseEntity<List<ProductListItemDTO>> getLowStockProducts(
            @Parameter(description = "Maximum number of products to return") @RequestParam(required = false, defaultValue = "5") int limit) {

        List<ProductListItemDTO> products = productService.getLowStockProducts(limit);
        return new ResponseEntity<>(products, HttpStatus.OK);
    }

    @Operation(
            summary = "Get all low stock products with pagination",
            description = "Retrieves all products with low stock with pagination support. Used for 'View All Low Stock Products' functionality.",
            responses = {
                    @ApiResponse(
                            responseCode = "200",
    ...
    public ResponseEntity<Paginated<ProductListItemDTO>> getAllLowStockProducts(
        ...
        Paginated<ProductListItemDTO> products = productService.getAllLowStockProducts(pageable);
        return new ResponseEntity<>(products, HttpStatus.OK);
    }

    @Operation(
            summary = "Get top products by monthly revenue",
            description = "Retrieves top-performing products by revenue for a specific time period. Used for dashboard analytics.",
            responses = {
                    @ApiResponse(
                            responseCode = "200",
                            description = "List of top products by revenue",
                            content = @Content(
                                    mediaType = "application/json",
                                    schema = @Schema(implementation = ProductStatsSummaryDTO.class)
                            )
                    )
            }
    )
    ...
    @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
    public ResponseEntity<List<ProductStatsSummaryDTO>> getTopProductsByMonthlyRevenue(
        ...
        List<ProductStatsSummaryDTO> products = productService.getTopProductsByMonthlyRevenue(startDate, endDate, limit);
        return new ResponseEntity<>(products, HttpStatus.OK);
    }
